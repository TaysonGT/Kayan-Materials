    ...
    <div className="shadow-sm border border-[#d9d9d9] rounded-sm bg-white">
        <div className="p-4">
            <div className='flex justify-between items-center mb-2'>
                <div className="font-semibold">المواد</div>
                    ...
                    <table className="min-w-full">
                        <thead className="bg-gray-100">
                            <tr>
                                ...
                                <th className="px-4 py-2 font-semibold text-start">الوصف</th>
                            </tr>
                        </thead>
                        <tbody>
                            {data.length === 0 ? (
                                <tr>
                                    <td colSpan={2} className="text-center py-3">لا توجد مواد لعرضها</td>
                                </tr>
                            ) : (
                                data.slice(0, 4).map((row) => (
                                    <tr key={row.id} className="hover:bg-gray-50">
                                        <td className="px-4 py-2">{row.name}</td>
                                        <td className="px-4 py-2">{row.description?.substring(0, 30) || '-'}</td>
                                    </tr>
                                ))
                            )}
                        </tbody>
